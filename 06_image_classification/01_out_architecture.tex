...
\definecolor{cAug}{RGB}{203,229,238}
...
\node[box, fill=cAug, above=7mm of inp]      (aug)  {\texttt{distort}\\[-1pt]{\scriptsize distorsione elastica, $\sigma=4$, $\alpha=34$}};
\node[box, fill=cIn, above=9mm of aug]       (pix)  {784 pixel\\[-1pt]{\scriptsize da 0.0 (bianco) a 1.0 (nero)}};
\node[box, fill=cLin, above=13mm of pix]     (l1)   {Linear\\[-1pt]{\scriptsize\texttt{nn.Linear(784, 800)}}};
\node[box, fill=cAct, above=13mm of l1]      (act)  {Tanh};
\node[box, fill=cLin, above=13mm of act]     (l2)   {Linear\\[-1pt]{\scriptsize\texttt{nn.Linear(800, 10)}}};
...
% ------------------------------------------------------------------- archi --
\draw[flow] (inp)  -- (aug);
\draw[flow] (aug)  -- (pix)  node[shp, midway] {$(B,\,784)$};
\draw[flow] (pix)  -- (l1)   node[shp, midway] {$(B,\,784)$};
\draw[flow] (l1)   -- (act)  node[shp, midway] {$(B,\,800)$};
\draw[flow] (act)  -- (l2)   node[shp, midway] {$(B,\,800)$};
\draw[flow] (l2)   -- (log)  node[shp, midway] {$(B,\,10)$};
\draw[flow] (log)  -- (loss);
\draw[flow] (loss) -- (outp);

% la scorciatoia: in valutazione le immagini non si deformano
\draw[flow, dashed, black!45] (inp.west) -- ++(-16mm,0)
  |- node[pos=0.25, below, font=\sffamily\scriptsize, text=black!55,
          text width=30mm, align=center, yshift=-1mm]
     {in valutazione\\si salta} (pix.west);

% ----------------------------------------------- come parte ogni layer, a sx --
\node[book, left=30mm of l1, text width=46mm, draw=black!25] (i1) {%
  \textbf{init}\; \texttt{kaiming\_normal\_}, gain $5/3$\\
  cio\`e $\sigma = \frac{5/3}{\sqrt{784}}$, bias a zero};
\node[book, left=30mm of l2, text width=46mm, draw=black!25] (i2) {%
...
\node[book, right=13mm of aug] (b0) {%
  \textbf{nel libro:} niente. Le 50.000 immagini sono sempre quelle, e la rete
  le impara a memoria. Qui il training set si rigenera a ogni epoca, e in 60
  epoche la rete non vede mai due volte la stessa immagine};
...
\node[note, below=14mm of inp, text width=126mm] (nota) {%
  \textbf{Il punto di questo schema \`e la casella in basso.} L'architettura \`e
  quella del capitolo e non cambia di una riga: quello che porta dal 98\% al
  \textbf{99.19\%} sono i dati. Le distorsioni elastiche sono di Simard,
  Steinkraus e Platt (ICDAR 2003) \textemdash{} campo di spostamenti casuali,
  sfocato con una gaussiana di deviazione $\sigma$, moltiplicato per $\alpha$,
  applicato con interpolazione bilineare. Nella tabella storica di MNIST, con
  \emph{questa} rete (784-800-10, cross-entropy), Simard riporta 1.6\% di errore
  senza distorsioni, 1.1\% con le affini e \textbf{0.7\% con le elastiche}: quasi
  quanto passare a una convnet, senza passarci.\\[3pt]
  \textbf{Quello che manca \`e assente apposta.} Una \texttt{nn.BatchNorm1d} fra
  il lineare e la \texttt{tanh} peggiora la validation: con \emph{un} layer
  nascosto la scala giusta si calcola a mano, e il problema che la batchnorm
  risolve nasce a cinquanta layer (\`e la conclusione di
  \texttt{03\_mlp/03\_batchnorm.py}). Le distorsioni \emph{affini} da sole danno
  98.98\% di validation, e aggiunte alle elastiche 99.15\% contro 99.23\%: sono
  il caso limite delle elastiche per $\sigma$ grande, quindi non aggiungono
  niente. Il \texttt{nn.Dropout} non serve pi\`u \textemdash{} non perch\'e non
  regolarizzasse, ma perch\'e non c'\`e pi\`u niente da regolarizzare: senza un
  training set fisso l'overfitting non si presenta.\\[3pt]
  \textbf{Il training}, che non si vede nel disegno: \texttt{AdamW} con
  \texttt{lr=1e-3} e cosine annealing, minibatch da 32, 60 epoche, 7 minuti su
  CPU. Si sceglie tutto sulla validation e si tengono i pesi dell'epoca
  migliore; il test si guarda una volta sola, alla fine.};
